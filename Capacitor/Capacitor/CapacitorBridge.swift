...
internal class CapacitorBridge: NSObject, CAPBridgeProtocol {
    ...
    var cordovaPluginManager: CAPCDVPluginManager?
    ...
    private var cordovaParser: CAPCDVConfigParser?
    ...
    init(with configuration: InstanceConfiguration, delegate bridgeDelegate: CAPBridgeDelegate, cordovaConfiguration: CAPCDVConfigParser, assetHandler: WebViewAssetHandler, delegationHandler: WebViewDelegationHandler) {
    ...
    func registerPlugins() {
        ...
        for classIndex in 0..<Int(numClasses) {
            if let aClass: AnyClass = classes[classIndex] {
                if class_getSuperclass(aClass) == CAPCDVPlugin.self {
    ...
    func registerCordovaPlugins() {
        ...
        cordovaPluginManager = CAPCDVPluginManager.init(parser: cordovaParser, viewController: self.viewController, webView: self.getWebView())
    ...
    func handleCordovaJSCall(call: JSCall) {
        ...
        if let plugin = self.cordovaPluginManager?.getCommandInstance(call.pluginId.lowercased()) {
            ...
            let pluginCall = CAPCDVInvokedUrlCommand(arguments: arguments,
