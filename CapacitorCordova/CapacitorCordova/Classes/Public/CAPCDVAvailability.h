...
#define CAPCDV_VERSION [NSString stringWithFormat:@"%d.%d.%d", \
    (CORDOVA_VERSION_MIN_REQUIRED / 10000),                 \
    (CORDOVA_VERSION_MIN_REQUIRED % 10000) / 100,           \
    (CORDOVA_VERSION_MIN_REQUIRED % 10000) % 100]

// Enable this to log all exec() calls.
#define CAPCDV_ENABLE_EXEC_LOGGING 0
#if CAPCDV_ENABLE_EXEC_LOGGING
    #define CAPCDV_EXEC_LOG NSLog
#else
    #define CAPCDV_EXEC_LOG(...) do { \
} while (NO)
#endif
